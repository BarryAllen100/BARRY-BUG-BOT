   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./BARRY-BUG-BOT")(XeonBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
			...
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`𝙱𝙰𝚁𝚁𝚈-𝙱𝚄𝙶-𝙱𝙾𝚃`)}\n\n`))
            console.log(chalk.cyan(`< ================================================== >`))
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: SSK-FAMILYCAMPAGNY`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: BarryAllen100`))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @sasakicompagnie `))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER: 242067274660`))
            console.log(chalk.magenta(`${themeemoji} CREDIT: 𝙱𝚊𝚛𝚛𝚢 𝙰𝚕𝚕𝚎𝚗\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
