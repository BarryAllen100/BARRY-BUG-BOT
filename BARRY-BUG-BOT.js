...
"previewType": "PHOTO",
"thumbnailUrl": ``,
"thumbnail": fs.readFileSync(`./barryMedia/thumb.jpg`),
"sourceUrl": `${link}`}}},
{ quoted: m})
}
...
await XeonBotInc.sendMessage(from, {text: xeonlod[i], edit: key });
        ...
        if (autobio) {
            XeonBotInc.updateProfileStatus(`𝙱𝚘𝚝 𝚎𝚗 𝚕𝚒𝚐𝚗𝚎 𝟸𝟺𝚑/𝟸𝟺 𝚎𝚝 𝟽𝚓/𝟽 𝚙𝚊𝚛 𝙱𝚊𝚛𝚛𝚢 𝙰𝚕𝚕𝚎𝚗 𝙱𝚘𝚝 𝙾𝚠𝚗𝚎𝚛 : ${ownername} 𝙿𝚘𝚞𝚛 𝚙𝚕𝚞𝚜 𝚍𝚎 𝚑𝚊𝚌𝚔𝚜, 𝚛𝚎𝚌𝚑𝚎𝚛𝚌𝚑𝚎𝚣 𝚜𝚞𝚛 𝚈𝚘𝚞𝚝𝚞𝚋𝚎 𝚂𝚂𝙺-𝙵𝙰𝙼𝙸𝙻𝚈𝙲𝙰𝙼𝙿𝙰𝙶𝙽𝚈`).catch(_ => _)
        }
...
if (budy === BhosdikaXeon) {
let buffer = fs.readFileSync(`./barryMedia/apk/${BhosdikaXeon}.apk`)
sendapk(buffer)
}
}
...
if (budy === BhosdikaXeon) {
let buffer = fs.readFileSync(`./barryMedia/zip/${BhosdikaXeon}.zip`)
sendzip(buffer)
}
}
...
if (budy === BhosdikaXeon) {
let buffer = fs.readFileSync(`./barryMedia/doc/${BhosdikaXeon}.pdf`)
senddocu(buffer)
}
}
    ...
    case 'autostatusview':{
                    ...
                    XeonBotInc.sendMessage(i, {
                        ...
                        contextInfo: {
                            externalAdReply: {
                                ...
                                thumbnailUrl: 'https://files.catbox.moe/5h021m.jpg',
            ...
            case 'getcase':
                if (!isCreator) return replygcxeon(mess.owner)
                const getCase = (cases) => {
                    return "case" + `'${cases}'` + fs.readFileSync("BARRY-BUG-BOT.js").toString().split('case \'' + cases + '\'')[1].split("break")[0] + "break"
                }
...
case 'ping': {
    ...
    await XeonBotInc.sendMessage(m.chat, {
        ...
        quoted: m
            ...
            case 'buyprem':
            case 'premium': {
                let teks = `*Salut ${pushname}\n Premium est payant ! \n Met ton numéro dans Premium json file! \n et dépôt à Barry Allen pour l'activer on Youtube channel ssk-familycampagny*`
                await XeonBotInc.sendMessage(m.chat, {
                    text: teks,
                    contextInfo: {
                        externalAdReply: {
            ...
            case 'runtime':
                ...
                XeonBotInc.sendMessage(m.chat, {
                    ...
                    contextInfo: {
                        externalAdReply: {
                            showAdAttribution: true,
                            title: `Yt SSK-FAMILYCAMPAGNY`,
                            body: `Love from Barry Allen 🩵`,
            ...
            case 'scriptbot':
                XeonBotInc.sendMessage(m.chat, {
                    text: `🌐 *_Git:_* https://github.com/BarryAllen100/BARRY-BUG-BOT\n\n🪀 *_Group:_* https://chat.whatsapp.com/IdB2EfQiNlKBekQrigN9m9\n\n🪀 *_Channel:_* https://whatsapp.com/channel/0029Vajrhmz96H4IsEjh4a41\n\n🩵 *_Dev:_* https://Wa.me//+242067274660`,
                    contextInfo: {
                        externalAdReply: {
                            showAdAttribution: true,
                            title: `SCRIPT OF Barry-MD is on YouTube SSK-FAMILYCAMPAGNY 🩵`,
                            body: `Love You bro 🩵`,
                            thumbnailUrl: 'https://files.catbox.moe/5h021m.jpg',
                            sourceUrl: global.link,
                            mediaType: 1,
...
break
            case 'addvideo': {
                ...
                VideoXeon.push(q)
                await fsx.copy(delb, `./barryMedia/video/${q}.mp4`)
            ...
            case 'delvideo': {
                ...
                fs.writeFileSync('./database/autoreply/video.json', JSON.stringify(VideoXeon))
                fs.unlinkSync(`./barryMedia/video/${q}.mp4`)
            ...
            case 'addimage': {
                ...
                let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted)
                ImageXeon.push(q)
                await fsx.copy(delb, `./barryMedia/image/${q}.jpg`)
            ...
            case 'delimage': {
                ...
                fs.writeFileSync('./database/autoreply/image.json', JSON.stringify(ImageXeon))
                fs.unlinkSync(`./barryMedia/image/${q}.jpg`)
            ...
            case 'addsticker': {
                ...
                let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted)
                StickerXeon.push(q)
                await fsx.copy(delb, `./barryMedia/sticker/${q}.webp`)
            ...
            case 'delsticker': {
                ...
                fs.writeFileSync('./database/autoreply/sticker.json', JSON.stringify(StickerXeon))
                fs.unlinkSync(`./barryMedia/sticker/${q}.webp`)
            ...
            case 'addvn': {
                ...
                await fsx.copy(delb, `./barryMedia/audio/${q}.mp3`)
            ...
            case 'delvn': {
                ...
                fs.writeFileSync('./database/autoreply/vn.json', JSON.stringify(VoiceNoteXeon))
                fs.unlinkSync(`./barryMedia/audio/${q}.mp3`)
...
let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted)
ZipXeon.push(teks)
await fsx.copy(delb, `./barryMedia/zip/${teks}.zip`)
...
fs.writeFileSync('./database/autoreply/zip.json', JSON.stringify(ZipXeon))
fs.unlinkSync(`./barryMedia/zip/${teks}.zip`)
...
let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted)
apknye.push(teks)
await fsx.copy(delb, `./barryMedia/apk/${teks}.apk`)
...
fs.writeFileSync('./database/autoreply/apk.json', JSON.stringify(ApkXeon))
fs.unlinkSync(`./barryMedia/apk/${teks}.apk`)
...
let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted)
docunye.push(teks)
await fsx.copy(delb, `./barryMedia/doc/${teks}.pdf`)
...
fs.writeFileSync('./database/autoreply/doc.json', JSON.stringify(DocXeon))
fs.unlinkSync(`./barryMedia/doc/${teks}.pdf`)
...
if (typemenu === 'v1') {
                    ...
                    XeonBotInc.sendMessage(m.chat, {
      video: fs.readFileSync('./barryMedia/thumb2.mp4'),
      gifPlayback: true,
      caption: xeonmenuoh,
      contextInfo: {
      externalAdReply: {
